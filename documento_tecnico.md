...
La plataforma implementa tres niveles de observabilidad: logging estructurado local, rastreo de errores con Sentry y monitoreo de rendimiento con New Relic. Los tres se activan de forma independiente mediante variables de entorno, sin modificar el código.

### Logging estructurado

`logging_config.py` configura el logger de Python con formato fijo antes de que arranque la app:

```
2026-06-12T18:00:01 | INFO     | app.main | GET /materias status=200 duration=12ms
```

El middleware HTTP en `main.py` registra **cada request** con método, path, status code y duración en milisegundos. Si el status code es ≥ 400, el log se emite en nivel `WARNING` para facilitar el filtrado en producción:
...
async def log_requests(request: Request, call_next):
    ...
    logger.log(level, "%s %s status=%d duration=%dms",
                request.method, request.url.path, response.status_code, duration_ms)
    return response
```

Los loggers de `uvicorn.access` y `sqlalchemy.engine` se silencian a nivel `WARNING` para evitar ruido de bajo nivel.

### Sentry — rastreo de excepciones

**Paquete:** `sentry-sdk[fastapi]==2.25.1` (integración nativa con FastAPI).

Sentry se inicializa en `main.py` **solo si** la variable de entorno `SENTRY_DSN` tiene valor. Si la variable está vacía (como en desarrollo local), el bloque se omite y no hay ningún overhead:
...
if SENTRY_DSN:
    sentry_sdk.init(
        ...
        traces_sample_rate=0.2,    # captura el 20% de las trazas de rendimiento
        send_default_pii=False,    # no envía datos personales a Sentry
    )
```

Cuando ocurre una excepción no manejada, Sentry la captura automáticamente y la envía al dashboard con stack trace completo, contexto del request y el entorno configurado. El endpoint de prueba permite verificar la integración sin necesidad de forzar un error real:
...
|---|---|
| `SENTRY_DSN` | DSN del proyecto en Sentry (vacío = desactivado) |
| `ENVIRONMENT` | Etiqueta del entorno (`development`, `production`) |

### New Relic — monitoreo de rendimiento (APM)

**Paquete:** `newrelic==13.0.1` (agente APM oficial).

New Relic se inicializa también de forma condicional, verificando que la license key esté presente:
...
Una vez activo, el agente instrumenta automáticamente FastAPI, SQLAlchemy y el servidor Uvicorn sin requerir cambios en los endpoints existentes. El decorador `@newrelic.agent.function_trace()` permite instrumentar funciones individuales para verlas como trazas separadas en el dashboard APM:
...
New Relic reporta automáticamente tiempos de respuesta por endpoint, throughput, tasas de error y uso de CPU/memoria del proceso.

**Variables de entorno:**

| Variable | Descripción |
|---|---|
| `NEW_RELIC_LICENSE_KEY` | License key de la cuenta New Relic (vacío = desactivado) |
| `NEW_RELIC_APP_NAME` | Nombre de la app en el dashboard (`plataforma-academica`) |

### Resumen de la estrategia

| Herramienta | Qué aporta | Cuándo se activa |
|---|---|---|
| **Logging (stdlib)** | Trazabilidad de cada request en tiempo real | Siempre |
| **Sentry** | Captura de excepciones con contexto completo | `SENTRY_DSN` presente |
| **New Relic** | Métricas de rendimiento y APM | `NEW_RELIC_LICENSE_KEY` presente |

La activación condicional garantiza que en desarrollo local el sistema corre sin dependencias externas, y en producción (Render) se habilitan ambas herramientas seteando las variables en los secrets del servicio.
